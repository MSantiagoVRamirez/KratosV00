...
const StockServiciosWidget: React.FC = () => {
  ...
  const loadProductos = async () => {
    try { const { data } = await ProductoService.leer(); if (Array.isArray(data)) setProductos(data) } catch {}
  }
  const loadPuntos = async () => {
    try { const { data } = await PuntoVentaService.getAll(); if (Array.isArray(data)) setPuntos(data) } catch {}
  }

  useEffect(() => { load(); loadProductos(); loadPuntos() }, [])

  const filtered = useMemo(() => {
    const base = items.filter(i => i.productoServicio === true)
  ...
  const columns: GridColDef[] = [
    { field: 'productoNombre', headerName: 'Servicio', flex: 1, minWidth: 180, renderCell: (p) => <span>{(p.row as Inventario).productoNombre ?? '-'}</span> },
  ...
  return (
    <div className='contenido'>
      <div id='inventario-servicios' className='bloque-formulario'>
        <div><h2>Stock de Servicios</h2></div>
        ...
        <div className='bloque-botones' style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <input
            ...
            style={{ maxWidth: 320 }}
            placeholder='Buscar servicio, código o sucursal'
            value={search}
            ...
            content={
              <div style={grid2ColStyle}>
                <div className='form-group' style={{ gridColumn: 'span 2' }}>
                  <label style={{ color: 'white' }} className='form-label required'>Servicio</label>
                  <select
                    className='form-control'
                    value={edited.productoId || 0}
                    onChange={(e) => setEdited(prev => ({ ...prev, productoId: Number(e.target.value), productoServicio: true }))}
                    required
                  >
                    <option value={0} disabled>Seleccione un servicio</option>
                    {productos.filter(p => p.productoServicio === true).map(p => (
        ...
        {modalType === 'delete' && (
          <ModalDialog
            ...
            onConfirm={() => { InventarioService.eliminar(deleteId).then(() => { closeModal(); load() }) }}
            closeModal={closeModal}
          />
        )}
      </div>
